...
export default function Landing() {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      const glass = heroGlassRef.current;
      if (glass) {
        glass.style.opacity = String(fade);
        glass.style.transform = `scale(${1 - 0.1 * t})`;
      }
    };
    onScroll();
    window.addEventListener("scroll", onScroll, { passive: true });
  ...
  return (
    <div className="relative overflow-x-clip text-slate-800">
      ...
      <section className="relative h-svh overflow-hidden">
        ...
        {/* Szklanka wody w centrum hero — za tekstem (z-[4]), nad glowem
            i wielkim napisem, ale pod pływającymi kroplami i tekstem. */}
        <div
          ref={heroGlassRef}
          aria-hidden
