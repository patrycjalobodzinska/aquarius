...
const BorderGlow: React.FC<BorderGlowProps> = ({
  ...
  useEffect(() => {
    ...
    const runSweep = () => {
      ...
      animateValue({ duration: 500, onUpdate: v => { if (!cancelled) setEdgeProximity(v / 100); } });
      ...
      animateValue({ ease: easeInCubic, delay: 2500, duration: 1500, start: 100, end: 0,
        onUpdate: v => { if (!cancelled) setEdgeProximity(v / 100); },
        onEnd: () => {
          if (cancelled) return;
          setSweepActive(false);
          timeoutId = setTimeout(runSweep, LOOP_GAP_MS);
        },
      });
    };
